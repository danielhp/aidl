...
Xvali = Xval[i, :, :, :]
Yvali = Yval[i, :, :].reshape(256, 512)

print(Xvali.shape)
print(Yvali.shape)

Ypred = model.predict(Xvali.reshape(1, 256, 512, 10))

fig = plt.figure(figsize=(20, 10), facecolor='w', edgecolor='k')
# plt.gray()
plt.subplot(221)
plt.title('Original map time_t-3')
plt.imshow(Xvali[:, :, 0])
plt.subplot(222)
plt.title('Original map time_t')
plt.imshow(Xvali[:, :, 9])
plt.subplot(223)
plt.title('Predicted map time_t+1')
